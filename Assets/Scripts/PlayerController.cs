...
public class PlayerController : MonoBehaviour
{
    ...
    public float slopedLimt = 45f;         // 동반 가능한 최대 경사 각도 
    ...
    void HandleRotation()
    {
        float mouseX = Input.GetAxis("MouseX") * mouseSenesitivity;      //마우스 좌우 입력
        float mouseY = Input.GetAxis("MouseY") * mouseSenesitivity;      //마우스 상하 입력 
    ...
    public void HandleMovement()
    {
        ...
        if (!isFirstPerson)//3인칭 모드일때 , 카메라 방향으로 이동처리
        {
            ...
            cameraForward.Normalize(); //방향 백터 정규화 (0~1) 사이 값으로 만들어준다.
